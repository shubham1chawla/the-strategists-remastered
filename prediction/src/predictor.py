...
class PredictorMetadata:
    ...
    def __init__(self, configuration: PredictorConfiguration, classifier: PredictorClassifier) -> None:
        ...
        self.classifier_type = classifier.prediction_classifier_type
        self.score = classifier.best_score
        self.params = classifier.best_params
...
class Predictor:
    ...
    def export_model(self) -> None:
        ...
        test_size, random_state = self.configuration_.TEST_SIZE, self.configuration_.RANDOM_STATE
        ...
        for classifier in get_predictor_classifiers(self.configuration_):

            n_splits = self.configuration_.KFOLD_N_SPLITS
            ...
            for _, (train_indexes, val_indexes) in enumerate(kf.split(x_train)):
                ...
                class_weight = self.configuration_.SAMPLE_WEIGHT_CLASS
            ...
            log.info(
                f'Classifier: {classifier.prediction_classifier_type.name:<18} | '
                f'Train Accuracy (Avg): {mean_train_accuracy:.2f} | '
                f'Validation Accuracy (Avg): {mean_val_accuracy:.2f}'
            )

            delta = abs(mean_val_accuracy - best_mean_val_accuracy)
            if (
                    (delta < 0.01 and mean_train_accuracy > best_mean_train_accuracy) or
                    (mean_val_accuracy > best_mean_val_accuracy)
            ):
        ...
        y_pred, y_pred_probs = best_classifier.predict(x_test), best_classifier.predict_proba(x_test)[:, 1]
        test_accuracy = accuracy_score(y_test, y_pred)
        roc_auc = roc_auc_score(y_test, y_pred_probs)

        # Preparing metadata
        metadata = PredictorMetadata(self.configuration_, best_classifier)
        ...
        # Logging metadata
        metadata.describe()
        if self.configuration_.show_plots:
            Predictor.plot_roc_curve(y_test, y_pred_probs, roc_auc)

        # Exporting metadata
        metadata.export()

        # Exporting model's pickle file
        pickle_file_path, search_method = self.configuration_.pickle_file_path, self.configuration_.search_method
        best_classifier.export(pickle_file_path)
        end_time = time.time() - start_time
        log.info(
            f'Export Path: {pickle_file_path} | '
            f'Export Time: {end_time:.02f} seconds | '
    ...
    def execute_model(self) -> None:
        classifier = PredictorClassifier.load(self.configuration_.pickle_file_path)
        log.info(f'Loaded classifier: {classifier.prediction_classifier_type.name}')

        test_file_path = self.configuration_.test_file_path
        ...
        for player_id, row in df.groupby(by='player.id'):
            player_id: int
            x = Predictor.extract_features(row)
        ...
        with open(prediction_file_path, 'w') as file:
            ...
            log.info(f'Exported predictions to: {prediction_file_path}')

    @staticmethod
    def extract_features(df: pd.DataFrame) -> pd.DataFrame:
        features = df.copy()
        features['ownership.average'] = (
                features['ownership.total'] / (features['ownership.count'] * 100)
        )
        features['debit.invest.average'] = (
                features['debit.invest.total'] / (features['debit.invest.count'] * df['player.base-cash'])
        )
        features['debit.rent.average'] = (
                features['debit.rent.total'] / (features['debit.rent.count'] * df['player.base-cash'])
        )
        features['credit.rent.average'] = (
                features['credit.rent.total'] / (features['credit.rent.count'] * df['player.base-cash'])
        )
        for col in df.columns:
            if col.startswith('ownership') and not (col.endswith('total') or col.endswith('count')):
                features[col] /= 100
            elif (
                    (col.startswith('debit') or col.startswith('credit')) and
                    not (col.endswith('total') or col.endswith('count'))
            ):
    ...
    def import_data(self) -> tuple[pd.DataFrame, int, int]:
        ...
        data_directory, csv_files, dfs = self.configuration_.data_directory, [], []
        for export_file_path in listdir(data_directory):
            if export_file_path.endswith('.csv'):
                csv_file: str = path.join(data_directory, export_file_path)
